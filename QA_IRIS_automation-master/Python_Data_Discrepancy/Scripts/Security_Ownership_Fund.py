"""
Checking if data (style, aum, equity_aum, turnover) on Security Ownership Page matches the Fund Profile Page
What the script does:
Looks at the all the funds in the ownership table in the Ownership Page (non-surveillance) and checks if the data (style, eAUM, AUM, turnover, position) on the Ownership Page
matches each and every fund profile page.
...
Output: A CSV with 3 columns, the factset_fund_id, fund_name, Discrepancy Column.
...
def main(argv):
	api_url = cfg.api_url
	ownership_columns = [
		'factset_fund_id', 'fund_name', 'style', 'turnover', 'equity_aum',
		'portfolio_value', 'market_value', 'current', 'quality_rating',
	]
	...
	result_df = pd.DataFrame(columns=['factset_fund_id','fund_name', 'Discrepency'])
	...
	result_df.to_csv("../Excel_SpreadSheets/Output_Data/Security_Ownership_Profile/APD_surveillance-off_fund_TEST.csv", index=False)
...
def ownership_comparison(item, auth_token, api_url):
	...
	if numpy.isnan(getattr(item , 'portfolio_value')):
		ownership_aum = "None"
	else:
		ownership_aum = str(int(getattr(item, 'portfolio_value')))
	ownership_turnover = str(getattr(item, 'turnover'))

	print(item)
	fund_id = getattr(item, 'factset_fund_id')

	fund_url = api_url + "api/fund/info/" + fund_id + "?_dc=1563905284318&appver=3.1.1&_dc=1563905284318"
	profile_resp = requests.get(fund_url, headers={"Authorization": auth_token})
	...
	if (ownership_style != profile_style) or (ownership_aum != profile_aum) or (
		...
		curr_fund_discrepancy += "Incorrect: "

	if ownership_style != profile_style:
		curr_fund_discrepancy += "style-(Ownership:" + ownership_style + " Profile: " + profile_style + ") "

	if ownership_aum != profile_aum:
		curr_fund_discrepancy += "aum-(Ownership:" + ownership_aum + " Profile: " + profile_aum + ") "

	if ownership_equity_aum != profile_equity_aum:
		curr_fund_discrepancy += "eaum-(Ownership:" + ownership_equity_aum + " Profile: " + profile_equity_aum + ") "

	if ownership_turnover != profile_turnover:
		curr_fund_discrepancy += "turnover-(Ownership:" + ownership_turnover + " Profile: " + profile_turnover + ") "
	...
	result_df = pd.DataFrame({
		"factset_fund_id": [fund_id],
		"fund_name": [str(getattr(item, 'fund_name'))],
		"Discrepency": [curr_fund_discrepancy]}, index=None)
	return result_df
